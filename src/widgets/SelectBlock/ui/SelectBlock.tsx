...
import { chartActions } from "shared/charts/areaChart/model/slices/slice"
...
export const SelectBlock = ({ interval, dayAgo }: SelectBlockType) => {
  ...
  return (
    <div className={s.block}>
      <div>
        <p>interval</p>
        <Select value={interval} items={INTERVAL_VALUE} setValue={(value) => {
          setInterval(value)
        }} />
      </div>
      <div>
        <p>period(days)</p>
        <Select value={dayAgo} items={DAYS_VALUE} setValue={(value) => {
          setHistoryDay(value)
        }} />
      </div>
    </div>
